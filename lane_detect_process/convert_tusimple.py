...
def draw(im, line, idx, shape, show=False):
    ...
    line_x = line[::2]
    line_y = line[1::2]
    pt0 = (int((line_x[0])/shape[1] *640), int(line_y[0]/shape[0] *352))
    ...
    for i in range(len(line_x) - 1):
        cv2.line(im, pt0, (int(line_x[i + 1]/shape[1] *640), int(line_y[i + 1]/shape[0] *352)), (idx,), thickness=2)  # thickness = i//4 +1
        # cv2.line(im, pt0, (int(line_x[i + 1]), int(line_y[i + 1])), (idx*20,), thickness=2)  # thickness = i//4 +1
        pt0 = (int(line_x[i + 1]/shape[1] *640), int(line_y[i + 1]/shape[0] *352))
...
def generate_segmentation_and_train_list(root, line_txt, names):
    ...
    for i in tqdm.tqdm(range(len(line_txt))):
        ...
        # label_path = names[i][:-4]+'_mask.png'
        shape = (720,1280)
        label = np.zeros((352,640),dtype=np.uint8)
        # label = cv2.imread(os.path.join(root,names[i]))
        ...
        labelpath = os.path.join(root, 'gt_image','%06d.png'%i)
        cv2.imwrite(labelpath,label)
        newimgpath = os.path.join(root,  'image', '%06d.jpg' % i)
        img = cv2.imread(os.path.join(root, names[i]))
        img = cv2.resize(img,dsize=None,fy=0.5,fx=0.5)[8:360,:]

        cv2.imwrite(newimgpath, img)


        train_gt_fp.write(os.path.join('image', '%06d.jpg' % i) + ' ' + os.path.join('gt_image','%06d.png'%i) + ' '+' '.join(list(map(str,bin_label))) + '\n')
    train_gt_fp.close()
...
def get_args():
    ...
    parser.add_argument('--root',default='/home/wqg/data/tusimple',  help='The root of the Tusimple dataset')
    return parser
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(args.root, 'image'), exist_ok=True)
