...
def generate_segmentation_and_train_list(root, line_txt, names):
    ...
    for i in tqdm.tqdm(range(len(line_txt))):
        ...
        labelpath = os.path.join(root, 'auto_gt_image', '%06d.png' % i)
        cv2.imwrite(labelpath, label)
        newimgpath = os.path.join(root, 'auto_images', '%06d.jpg' % i)

        cv2.imwrite(newimgpath, img)

        train_gt_fp.write(
            os.path.join('tusimple', 'auto_images', '%06d.jpg' % i) + ' ' + os.path.join('tusimple', 'auto_gt_image', '%06d.png' % i) + ' ' + ' '.join(
                list(map(str, bin_label))) + '\n')
    train_gt_fp.close()
